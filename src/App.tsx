...
const personaMeta: Record<string, { emoji: string; tone: string; color: string }> = {
  'Lowball Larry': { emoji: '🧢', tone: 'Direct negotiator', color: 'text-amber-300' },
  'Bundle Bella': { emoji: '✨', tone: 'Bundle hunter', color: 'text-fuchsia-300' },
  'Flash Frank': { emoji: '⚡', tone: 'Fast close', color: 'text-cyan-300' },
};
...
function App() {
  ...
  const pending = useStore((s) => s.pendingDeals);
  const analytics = useStore((s) => s.analytics());
  const respond = useStore((s) => s.respond);
  const markApproval = useStore((s) => s.markHumanApproval);
  const approve = useStore((s) => s.approveDeal);
  const reject = useStore((s) => s.rejectDeal);
  const setItemPrice = useStore((s) => s.setItemPrice);
